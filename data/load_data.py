...
from models.utils import Rotation, Zoom
...
def load_data_subset(data_aug, batch_size,workers,dataset, data_target_dir, validation_fraction, labels_per_class=100):
    ...
    from functools import reduce
    from operator import __or__
    from torch.models.data.sampler import SubsetRandomSampler
    ...
    if validation_fraction > 0.0:
        ...
        valid = torch.models.data.DataLoader(train_data, batch_size=batch_size, sampler=valid_sampler,  num_workers=workers, pin_memory=True)
    else:
        train_sampler = get_sampler(train_data.train_labels, labels_per_class, validation_fraction)
        valid = None

    labelled = torch.models.data.DataLoader(train_data, batch_size=batch_size, sampler=train_sampler,  num_workers=workers, pin_memory=True)


    unlabelled = torch.models.data.DataLoader(train_data, batch_size=batch_size, sampler=get_sampler(train_data.train_labels),  num_workers=workers, pin_memory=True)
    test = torch.models.data.DataLoader(test_data, batch_size=batch_size, sampler=get_sampler(test_data.test_labels), num_workers=workers, pin_memory=True)
